...
import com.example.cocomo_ii.SecondFp_ViewBinding;
import com.example.cocomo_ii.FP_ACTIVITY;

public class SecondFp extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private double UFP = FP_ACTIVITY.internal_logical_file_final + FP_ACTIVITY.external_logical_file_final +
            FP_ACTIVITY.external_input_final + FP_ACTIVITY.external_output_final + FP_ACTIVITY.external_inquiry_final;

    private double CAF = 0.65 + 0.01 * FP_ACTIVITY.complexity_factor;
    private double FP = UFP * CAF;
    private double mKLOC = (FP * FP_ACTIVITY.lang_final) / 1000;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_second_fp);
        ...
        ComputeEstimateDevelopment.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                calculateEstimate(value);
                // Toast.makeText(SecondFp.this, "FP  = " + String.valueOf(FP), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void calculateEstimate(int value) {
        ...
        if (FP == 0 || costinput.isEmpty()) {
            Toast.makeText(this, "please check language factor or cost input", Toast.LENGTH_SHORT).show();
        } else {
            switch (value) {
                case R.id.organic:
        ...
        if (parent.getId() == R.id.spinner_RSR) {
            ...
            if (!TextUtils.isEmpty(selection)) {
                if (selection.equals(getString(R.string.cs_verylow))) {
                    Toast.makeText(SecondFp.this, "FP " + String.valueOf(FP_ACTIVITY.lang_final), Toast.LENGTH_SHORT).show();
                    cs_1 = 0.75;
